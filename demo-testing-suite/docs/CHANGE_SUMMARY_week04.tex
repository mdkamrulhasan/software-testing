...
transcription of the reading's \texttt{PasswordResetService} /
\texttt{smtplib} worked example from ``Dependency Isolation and
Seams'': a dependency-injection seam (\texttt{email\_client}) replacing
a hard-coded dependency. \\
\texttt{tests/notifications/test\_password\_reset\_service.py} & New &
One test proving the injected seam works, plus the reading's ``mocks
that lie'' pitfall, as three runnable \texttt{pytest} tests. \\
\texttt{app/orders/order\_service.py} & New & A direct transcription of
the reading's separate ``Test Double Taxonomy'' worked example: an
order-placement flow with three collaborators (payment, database,
email). \\
\texttt{tests/orders/test\_order\_service.py} & New & The reading's full
dummy/stub/fake/spy/mock taxonomy, as six runnable \texttt{pytest}
tests, one per collaborator scenario the reading walks through. \\
...
\begin{keyidea}
The reading updated its ``Test Double Taxonomy'' section after this
codebase's first Week 4 pass, switching from illustrating all five
doubles against \texttt{PasswordResetService} to a separate
order-placement example. This revision follows that update: the seam
worked example (\texttt{password\_reset.py}) and the taxonomy worked
example (\texttt{order\_service.py}) are now two distinct files, matching
the reading's own two distinct sections.
\end{keyidea}
...
% ============================================================
\section{New Tests: Proving the Seam Works}
% ============================================================
\textbf{Location:} \texttt{tests/notifications/test\_password\_reset\_service.py}

Since the reading's Test Double Taxonomy no longer uses
\texttt{PasswordResetService}, this file keeps only what the
``Dependency Isolation and Seams'' section itself calls for: one test
proving the injected \texttt{email\_client} seam works with ``any object
that has a \texttt{.send(to, message)} method,'' exactly as the reading
describes.
...
The file also carries the reading's ``Mocks that lie'' pitfall (Common
Pitfalls, below), since \texttt{EmailClient} --- defined in
\texttt{password\_reset.py} --- is a convenient real interface to
demonstrate it against.

% ============================================================
\section{New App Code and Tests: The Test Double Taxonomy}
% ============================================================
\textbf{Locations:} \texttt{app/orders/order\_service.py},
\texttt{tests/orders/test\_order\_service.py}

The reading illustrates all five doubles with, in its own words, ``a
single running example: an order\_service that, when placing an order,
normally has to interact with three external collaborators --- a
payment service, a database, and an email service.''
\texttt{order\_service.py} transcribes that example's three functions
verbatim: \texttt{calculate\_total(price, logger)},
\texttt{place\_order(payment\_service)}, and
\texttt{register\_user(email\_service)}.
\texttt{test\_order\_service.py} encodes all five doubles as six
runnable tests, in the reading's own order:

\begin{itemize}
\item \textbf{Dummy} --- a plain \texttt{DummyLogger} against
\texttt{calculate\_total()}, plus \texttt{PoisonedDummyLogger} for the
reading's second Dummy tip box (any accidental use raises immediately).
\item \textbf{Stub} --- \texttt{PaymentStub}, feeding
\texttt{place\_order()}; no assertion is made about how
\texttt{process\_payment()} was called.
\item \textbf{Fake} --- \texttt{FakeDatabase}, an in-memory dictionary,
verified via \textbf{state verification}
(\texttt{fake\_db.get\_user(...)} after the fact).
\item \textbf{Spy} --- \texttt{EmailSpy}, feeding
\texttt{register\_user()}; records \texttt{sent\_to} for the test to
inspect afterward.
\item \textbf{Mock} --- a bare \texttt{Mock()}, feeding
\texttt{place\_order()} again, verified via \textbf{behavior
verification} (\texttt{assert\_called\_once\_with(100)}).
\end{itemize}
...
\end{lstlisting}
All six tests pass. Unlike Weeks 1 and 3, there is no deliberate bug to
find in this file --- the teaching payload is in reading the five
doubles side by side, not in a failure. Notice that Stub and Mock both
drive the \emph{same} \texttt{place\_order()} function; only what the
test checks afterward differs --- the reading's own way of distinguishing
the two.
\end{tipbox}
...
reflect the six new app/test files above and to describe the suite's
current expected output. If you run the full suite from the project
root:
...
you should now see \textbf{45 passed, 2 xfailed, 1 failed} --- twelve
more passing tests than Week 3's baseline, and the same two
\texttt{xfail} results and one \texttt{FAIL} as before, all unrelated to
...
\texttt{app/notifications/password\_reset.py},
\texttt{app/orders/order\_service.py}, and
\texttt{app/accounts/registration\_service.py} exist so you can see all
three pieces of that argument against real, running code: a seam
introduced where a hard-coded dependency used to be, all five kinds of
double filling seams in five different tests, and a real caller mocked
in isolation from its dependency.
\end{keyidea}
...
solves it.
\item Run \texttt{pytest tests/orders/test\_order\_service.py -v} and,
for each of the five doubles, say out loud which school (classical or
mockist) it belongs to and why.
